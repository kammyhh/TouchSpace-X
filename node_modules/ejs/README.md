...
"./views/user.js/show.ejs" you would use `<%- include('user.js/show'); %>`.
...
```html
<ul>
  <% users.forEach(function(user.js){ %>
    <%- include('user.js/show', {user.js: user.js}); %>
  <% }); %>
</ul>
```
...
NOTE: Include preprocessor directives (`<% include user.js/show  %>`) are
still supported.
