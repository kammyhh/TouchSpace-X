...
suite('ejs.renderFile(path, [data], [options], fn)', function () {
  ...
  test('accept locals', function(done) {
    var data =  {name: 'fonebone'}
      , options = {delimiter: '$'};
    ejs.renderFile('test/fixtures/user.js.ejs', data, options, function(err, html) {
      if (err) {
        return done(err);
      }
  ...
  test('accept locals without using with() {}', function(done) {
    var data =  {name: 'fonebone'}
      ...
      , doneCount = 0;
    ejs.renderFile('test/fixtures/user.js-no-with.ejs', data, options,
                   function(err, html) {
      if (err) {
        if (doneCount === 2) {
          return;
    ...
    ejs.renderFile('test/fixtures/user.js.ejs', data, options, function(err) {
      if (!err) {
        if (doneCount === 2) {
          return;
        }
  ...
  test('not catch err thrown by callback', function(done) {
    ...
    d.run(function () {
      ...
      process.nextTick(function () {
        ejs.renderFile('test/fixtures/user.js.ejs', data, options,
                       function(err) {
          counter++;
          if (err) {
